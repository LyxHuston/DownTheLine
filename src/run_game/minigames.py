...
from run_game import entities, items, gameboard
...
class Minigame:
	...
	funcs = ("init", "setup", "tick", "check_win", "finish")
...
def filter_entities(area):
	area.entity_tracker[:] = filter(lambda e: e.alive, area.entity_tracker)
...
@fish.set_check_win
def empty_area(area):
	return not area.entity_tracker


@fish.set_setup
def fish_setup(area):
	register = area.entity_tracker.append
...
@notes.set_init
def notes_init(area):
	area.length = game_states.HEIGHT
...
def lazers_setup(area):
	...
	for i in range(area.difficulty // 10):
		...
		wave = [(entities.MassDelayedDeploy, (delay, area, wave, register))]
	e = wave[0][0](*wave[0][1])
	gameboard.NEW_ENTITIES.append(e)
	register(e)
